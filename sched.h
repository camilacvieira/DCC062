...
//             de um processo especificamente para o algoritmo
...
//- setSomeFeatureFn e'a funcao que permite modificar alguma caracteristica 
//             do algoritmo de escalonamento para um dado processo
//- notifyProcessStatusFn e'a funcao a receber uma notificacao de que o status
//             de um processo mudou para alguma manutencao de dados do algoritmo
typedef struct sched_info {
        char name[MAX_NAME_LEN+1];
        void (*initParamsFn)(Process *p, void *params);
        Process* (*scheduleFn)(Process *plist);
        int (*releaseParamsFn)(Process *p);
	void (*setSomeFeatureFn)(Process *p, void* featureData);
	void (*notifyProcessStatusFn)(Process *p, int oldStatus);
} SchedInfo;
